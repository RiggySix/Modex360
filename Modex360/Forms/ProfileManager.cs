...
namespace Modex360
{
    public partial class ProfileManager : Office2007Form
    {
        ...
        private static byte[] createSerializedDataHeader()
        {
            ...
            string headerString = String.Format("Modex360, Version={0}, Culture=neutral, PublicKeyToken=6d2e1f41111aca92", Config.clientVersion);
            io.Out.Write(headerString.Length);
            io.Out.Write(Encoding.ASCII.GetBytes(headerString));
            io.Out.Write(new byte[] { 0x04, 0x01 });
            headerString = String.Format("System.Collections.Generic.List`1[[Modex360.ProfileData, Modex360, Version={0}, Culture=neutral, PublicKeyToken=6d2e1f41111aca92]]", Config.clientVersion);
            io.Out.Write(headerString.Length);
        ...
        private void cmdAddProfile_Click(object sender, EventArgs e)
        {
            ...
            (addThread = new Thread((ThreadStart)delegate
            {
                try
                {
                    ...
                    {
                        if (UI.messageBox("This account already exists in Modex360.\n\nDo you want to replace it?", "Replace Profile?",
