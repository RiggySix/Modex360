...
using Modex360.Library.Systems.FATX;
using XContent;
using Modex360.PackageEditors.Package_Manager;
...
namespace Modex360
{
    internal static class FormHandle
    {
        ...
        {
            foreach (KeyValuePair<int, FormConfig> fc in Forms)
                if (fc.Value.DeviceIndex == x && fc.Value.ActiveForm is PackageEditors.EditorControl
                    && ((PackageEditors.EditorControl)fc.Value.ActiveForm).WorkerRunning)
                {
        ...
        {
            int x = createNewFormConfig(Modex360.FormConfig.getFormMetaIndex(FormID.PackageManager), -1, null, false);
        ...
        internal class FormConfig
        {
            ...
            internal Modex360.FormConfig.FormMeta Meta
            {
                get { return Modex360.FormConfig.formList[MetaIndex]; }
            }

            internal IEditorControl ActiveForm
            {
            ...
            internal void KillLastForm(IEditorControl currentControl)
            {
                KillLastForm();
