    ...
    public partial class DMC3 : EditorControl
    {
        ...
        private void btnClick_ClearSlotData(object sender, EventArgs e)
        {
            if (this.CurrentSlot != null && !this.CurrentSlot.IsEmpty)
            {
                ...
                {
                    Modex360.Functions.UI.messageBox("You cannot delete the only save slot.");
                }
            }
            else
            {
                Modex360.Functions.UI.messageBox("Cannot access the current slot.");
            }
        }
        ...
        private void btnClick_CopySlotData(object sender, EventArgs e)
        {
            ...
            if (this.CurrentSlot != null && !this.CurrentSlot.IsEmpty)
            {
                ...
                if (oldSlot != newSlot)
                {
                    ...
                    Modex360.Functions.UI.messageBox(string.Format("Successfully copied slot {0} to slot {1}!", oldSlot, newSlot));
                ...
                {
                    Modex360.Functions.UI.messageBox("You cannot copy a slot to the same slot number.");
                }
            }
            else
            {
