    ...
    public partial class ForzaHorizonProfile : EditorControl
    {
        ...
        private bool LoadSaveData()
        {
            try
            {
                ...
                this.numCredits.Value = _forzaProfile.SaveIO.In.ReadUInt32();
        ...
        private static string FormatSettingValue(ForzaProfileEntry entry)
        {
            switch (entry.Type)
            {
                ...
                case ForzaTypes.UInt32:
                    return BitConverter.ToUInt32(Modex360.Functions.Global.convertToBigEndian(entry.Value), 0).ToString();
                case ForzaTypes.Bool:
                    return BitConverter.ToBoolean(Modex360.Functions.Global.convertToBigEndian(entry.Value), 0).ToString();
                case ForzaTypes.Uint16:
                    return BitConverter.ToUInt16(Modex360.Functions.Global.convertToBigEndian(entry.Value), 0).ToString();
                case ForzaTypes.UInt8:
                    return entry.Value[0].ToString();
                case ForzaTypes.Float32:
                    return BitConverter.ToSingle(Modex360.Functions.Global.convertToBigEndian(entry.Value), 0).ToString();
                case ForzaTypes.UInt64:
                    return BitConverter.ToUInt64(Modex360.Functions.Global.convertToBigEndian(entry.Value), 0).ToString();
                default:
        ...
        {
            this.numXP.Value = _playerLevels[this.cmbLevel.SelectedIndex];
        }

        [Conditional("INT2")]
        private void BtnClickExtractProfile(object sender, EventArgs e)
        {
        ...
        {
            ForzaCarbinWriter writer = new ForzaCarbinWriter(_forzaDatabase.RetrieveTableData("Career_Garage"), _carbinKey);
            writer.ExportToDirectory(@"C:\Users\Thierry\Desktop\Game Projects\Forza\Forza Horizon\Saves\Garage");
        }

        private void InitializeGarageInfo()
        {
            _unicornCarListing = new[]
                                     {
            ...
            _carIds.AddRange(_unicornCarIds);
            ...
            // Add the unicorn cars to the main car lists
            for (var i = 0; i < _unicornCarListing.Length; i++)
            {
                _carNameList.Add(_unicornCarIds[i], _unicornCarListing[i]);
                _carThumbNameList.Add(_unicornCarIds[i], _unicornCarListing[i].Replace(' ', '_') + ".xdc");
                _carIdList.Add(_unicornCarListing[i], _unicornCarIds[i]);
            }
            // Add the secret cars to the main car lists
            for (var i = 0; i < _secretCarListing.Length; i++)
            {
                _carNameList.Add(_secretCarIds[i], _secretCarListing[i]);
                _carThumbNameList.Add(_secretCarIds[i], _secretCarListing[i].Replace(' ', '_') + ".xdc");
                _carIdList.Add(_secretCarListing[i], _secretCarIds[i]);
            }

            // Add the unicorn and secret cars to the drop-box for user editing
            if (clmnCarList.Items.Count == 0x00)
            {
                clmnCarList.Items.AddRange(_unicornCarListing);
        ...
        private void BtnClickAddAllUnicornCarsToGarage(object sender, EventArgs e)
        {
            AddCarsListToGarage(_unicornCarIds);
        }

        private void BtnClickAddAllSecretCarsToGarage(object sender, EventArgs e)
        {
