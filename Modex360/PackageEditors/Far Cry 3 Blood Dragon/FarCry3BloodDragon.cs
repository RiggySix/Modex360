...
namespace Modex360.PackageEditors.Far_Cry_3_Blood_Dragon
{
    public partial class FarCry3BloodDragon : EditorControl
    {
        private FarCry3Save SaveGame;
        private List<fValue> _saveProperties;
        private static string _campaignSavePlugin;

        //private uint[] _ammoPathIds, _ammoAccessIds;
        CustomClass myProperties = new CustomClass();

        public FarCry3BloodDragon()
        {
            InitializeComponent();
            TitleID = FormID.FarCry3BloodDragon;
        }
        ...
        {
            var parser = new XmlParser(_campaignSavePlugin, false);
            _saveProperties = parser.ValuesList;
            foreach (fPropertyBag propertyBag in _saveProperties)
            {
                var bagAccess = FarCry3Attribute.GetAccessIdsFromPath(string.Format("{0}\\{1}", "CampaignSave", propertyBag.Name));
