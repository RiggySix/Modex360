...
namespace CoDGhosts
{
    public class CampaignSave
    {
        ...
        public CampaignSave(EndianIO io)
        {
            ...
            uint size = IO.In.SeekNReadUInt32(0x48C);
            IO.In.SeekTo(0x500);
            byte[] SaveData = IO.In.ReadBytes(size);
            uint StoredSum = IO.In.SeekNReadUInt32(0x08);
            ...
            SaveIO = new EndianIO(SaveData, EndianType.BigEndian, true);
        }
        ...
        public byte[] Save(byte[] SaveData)
        {
            var io = new EndianIO(new MemoryStream(), EndianType.BigEndian, true);
            IO.In.SeekTo(0);
            byte[] header = IO.In.ReadBytes(0x500);
            io.Stream.SetLength(SaveData.Length + 0x500);
            io.Out.Write(header);
            io.Out.SeekNWrite(0x48C, SaveData.Length);
