    ...
    public class SaintsRow3GameSave
    {
        ...
        private void Read()
        {
            ...
            for (int x = 0; x < 0x32; x++)
            {
                ...
                if ((ident = IO.In.ReadUInt32()) != 0)
                {
                    ...
                    WeaponUpgradeTable.Add(ident, Modex360.Functions.BitHelper.ProduceBitmask(IO.In.ReadInt32()));
                }
            }
        ...
        private void Weapons_MaxAllWeaponLevels()
        {
            foreach (KeyValuePair<uint, string> weapon in this.WeaponList)
            {
                ...
                {
                    this.WeaponUpgradeTable.Add(weapon.Key, Modex360.Functions.BitHelper.ProduceBitmask(0x07));
                }
                else
                {
                    this.WeaponUpgradeTable[weapon.Key] = Modex360.Functions.BitHelper.ProduceBitmask(0x07);
                }
            }
        }
        private void Weapons_WriteWeaponLevels()
        {
            for (int x = 0; x < this.WeaponUpgradeTable.Count; x++)
            {
                ...
                IO.Out.Write(Modex360.Functions.BitHelper.ConvertToWriteableInteger(this.WeaponUpgradeTable.ElementAt(x).Value));
            }
        }
