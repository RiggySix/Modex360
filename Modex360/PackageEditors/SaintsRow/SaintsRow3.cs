                ...
                foreach (DataGridViewRow row in dtgWeaponsList.Rows)
                {
                    if (!row.IsNewRow)
                    {
                        ...
                        if (level > 0)
                        {
                            ...
                            {
                                this.GameSave.WeaponUpgradeTable[WeaponIdent] = Modex360.Functions.BitHelper.ProduceBitmask(level);
                            }
                            else
                            {
                                this.GameSave.WeaponUpgradeTable.Add(WeaponIdent, Modex360.Functions.BitHelper.ProduceBitmask(level));
                            }
                        }
                    }
                }
        ...
        private void BtnClick_InjectVehicle(object sender, EventArgs e)
        {
            ...
            for (int x = 0; x < this.dtgGarageList.SelectedRows.Count; x++)
            {
                ...
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        Modex360.Functions.UI.messageBox(string.Format(
            ...
            if(!Failed)
            {
                Modex360.Functions.UI.messageBox(string.Format("Successfully injected {0} files!", this.dtgGarageList.SelectedRows.Count));
            }
        }
