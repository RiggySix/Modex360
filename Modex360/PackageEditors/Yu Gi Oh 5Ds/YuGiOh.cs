...
namespace YuGiOhDecadeDuels
{
    internal class CardListEntry
    {
        ...
        {
            Unlocked = Modex360.Functions.BitHelper.LoadValue((mask >> 0x18) & 0xFF, 8);
            New = Modex360.Functions.BitHelper.LoadValue((mask >> 0x10) & 0xFF, 8);
        }

        public byte[] ToArray()
        {
            byte[] card = new byte[4]; // should not be more than 32 bits
            card[0] = Modex360.Functions.BitHelper.ConvertToWriteableByte(Unlocked.ToArray());
            card[1] = Modex360.Functions.BitHelper.ConvertToWriteableByte(New.ToArray());
